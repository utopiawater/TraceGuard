...
export function DashboardPage() {
  ...
  return <><PageHeader title="安全态势总览" description="当前 run 的总体摘要：多源接入、标准化、Detection 与 AttackChain 覆盖。" aside={data ? <span className="freshness"><CheckCircle2 size={15}/> {runId ? `任务 ${runId}` : pipelineStatusText}</span> : undefined} />
    ...
    {data && <>
      ...
      <section className="chain-list"><div className="section-heading"><h2>最近攻击链</h2><span>候选链不会自动标记为已确认</span></div>{data.recent_chains.length ? data.recent_chains.map(chain=>{const stages=[...(new Set(chain.steps?.map(step=>stageName[step.stage]??step.stage).filter(Boolean)??[]))];const facts=[typeof chain.evidence_ids?.length==='number'?`${chain.evidence_ids.length} 条证据`:null,typeof chain.entity_ids?.length==='number'?`${chain.entity_ids.length} 个实体`:null].filter(Boolean);const sources=chain.sources?.map(source=>sourceDisplayName[source]??source).join(' / ');return <a className="chain-card" href={runScopedPath('/chains', { chain: chain.chain_id })} key={chain.chain_id}><span className="chain-score" title="链可信评分：由步骤检测置信度和 ATT&CK 映射置信度综合得到。"><strong>{pct(chain.score)}</strong><small>链可信评分</small></span><div className="chain-card-main"><div className="chain-card-head"><strong>{chain.title}</strong><span className={`badge ${chain.status==='candidate'?'candidate':'neutral'}`}>{chainStatusName[chain.status]??chain.status}</span></div>{stages.length>0&&<small className="chain-stage-path">{stages.join(' → ')}</small>}<small className="chain-technique-path">{chain.technique_ids.join(' → ')}</small>{(facts.length>0||sources)&&<small className="chain-card-meta">{facts.join(' · ')}{facts.length>0&&sources?' · ':''}{sources}</small>}</div></a>}) : <p className="inline-empty">暂无攻击链。检测结果形成可验证关联后会出现在这里。</p>}</section>
    </>}
  </>
}
