...
export function AgentsPage(){
  ...
  return <><PageHeader title="Agent 调查中心" description="从既有攻击链启动受约束调查，查看任务编排、只读工具调用、结构化 Findings 与证据校验结果。" />
    ...
    {!!items.length&&<div className="agent-workspace"><aside className="investigation-list panel"><div className="panel-title"><h2>调查列表</h2><span>{items.length} 次</span></div>{items.map(item=><button className={selectedCase===item.case_id?'selected':''} key={item.case_id} onClick={()=>setParams({case:item.case_id})}><span className={`status-pill ${item.status}`}>{statusName[item.status]}</span><time>{item.created_at?new Date(item.created_at).toLocaleString('zh-CN'):'—'}</time><strong>{item.case_id}</strong><small>{item.attack_chain}</small><span>{Math.round(item.final_confidence*100)}% 置信度 · {item.task_count} 节点</span>{item.model_fallback&&<em>deterministic fallback</em>}</button>)}</aside>
