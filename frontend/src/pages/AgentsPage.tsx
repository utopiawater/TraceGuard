...
export function AgentsPage(){
  ...
  const start=async(scope:'quick'|'full')=>{if(!chainId)return;setStarting(scope);setError(null);try{const maxSteps=scope==='quick'?4:12;const value=await apiPost<{case_id:string}>(`/api/chains/${chainId}/investigate?scope=${scope}&max_steps=${maxSteps}`);setParams({case:value.data.case_id});await load()}catch(reason){setError((reason as Error).message)}finally{setStarting(null)}}
  ...
  return <><PageHeader title="Agent 调查中心" description="从既有攻击链启动受约束调查，查看任务编排、只读工具调用、结构化 Findings 与证据校验结果。" />
    ...
    {!!items.length&&<div className="agent-workspace"><aside className="investigation-list panel"><div className="panel-title"><h2>调查列表</h2><span>{items.length} 次</span></div>{items.map(item=><button className={selectedCase===item.case_id?'selected':''} key={item.case_id} onClick={()=>setParams({case:item.case_id})}><span className={`status-pill ${item.status}`}>{taskStatusName[item.status]}</span><time>{item.created_at?new Date(item.created_at).toLocaleString('zh-CN'):'—'}</time><strong>{chains?.find(chain=>chain.chain_id===item.attack_chain)?.title??'攻击链调查'}</strong><small>{item.scope==='quick'?'快速调查 · 4 Agent':'完整调查 · 6 Agent'} · {item.case_id}</small><span title="Final Confidence 是已通过证据校验的 Findings 平均置信度，不代表攻击者身份确认。">{pct(item.final_confidence)} 最终置信度 · <b>{executionModeName[item.execution_mode] ?? item.execution_mode}</b></span></button>)}</aside>
